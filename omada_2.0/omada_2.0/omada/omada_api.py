...
#####################################################################################
    def Login(self):
        ...
        if self.loginResult == False:
            ...
            self.siteId = self.get_siteId(self.site)
...
#-----------------------------------------------------------------
    def Command(self,mod,api,verify=False, **arg):
...
##############################################################################
    ...
    def get_siteId(self, site):
        ...
        if not self.oauth_token:
             raise self._logger.error("OpenAPI not authenticated - cannot get siteId")
        
        url = f"{self.baseurl}/openapi/v1/{self.omadacId}/sites"
        ...
        for _site in result['data']:
            if _site['name'] == site:
                return _site['siteId'] 
        raise self._omadaError(f"Failed to get siteId")    
